// CommandSite pco-oauth-start Edge Function
// ---------------------------------------------------------------------------
// Returns the Planning Center consent URL a church should hit to authorize
// CommandSite. This is the multi-tenant replacement for a Personal Access
// Token: the church approves on PCO's own screen, and CommandSite receives a
// token scoped to THAT church at THAT staffer's permission level. Josh never
// needs admin on their PCO account.
//
...
//
// Secrets: PCO_OAUTH_CLIENT_ID, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
//
// Register the OAuth app once at:
//   https://api.planningcenteronline.com/oauth/applications
// with redirect URI = <SUPABASE_URL>/functions/v1/pco-oauth-callback
...
// Minimum scopes for the pilot. PCO scopes are product names. We deliberately
// do NOT request `giving` — the highest-sensitivity data — until giving is
// actually built, so it never sits in our token store before we need it.
const DEFAULT_SCOPE = ['people', 'check_ins', 'services', 'groups'].join(' ')

Deno.serve(async (req: Request) => {
  if (req.method === 'OPTIONS') return new Response(null, { headers: CORS })

  if (!CLIENT_ID) {
    return json({ error: 'PCO_OAUTH_CLIENT_ID secret not set in Supabase Edge Functions.' }, 500)
  }
  ...
  // Planning Center from their dashboard, but can never target another church.
  ...
  if (!isServiceRole) {
    ...
    if (role !== 'admin') {
      if (role !== 'client' || !clientId) {
        return json({ error: 'You do not have access to connect Planning Center.' }, 403)
      }
      ...
      if (!ownSlug || ownSlug !== tenant) {
        return json({ error: 'You can only connect Planning Center for your own church.' }, 403)
      }
    }
  }
  ...
  return json({
    auth_url: `https://api.planningcenteronline.com/oauth/authorize?${params.toString()}`,
    redirect_uri: REDIRECT_URI,
    scope,
    tenant,
  })
