...
function approve() {
  if (detail.value?.caseId) care.markHandled(detail.value.caseId)
  care.closeDetail()
}
function close() { care.closeDetail() }
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- The drafted note and its approval, next to the evidence that
           justifies it. These used to live in a separate queue at the top of
           the page, which meant approving from the one view that had LESS
           context than this one, and duplicated every card on screen. -->
      <div v-if="detail?.draft && !handled" class="border-t border-divider px-5 py-4">
        <div class="text-[11px] font-semibold uppercase tracking-[0.14em] text-brand">Grace drafted this note</div>
        <p class="mt-2 rounded-lg border border-divider bg-surface-elevated/50 px-3 py-2.5 text-[13px] italic leading-relaxed text-ink">
          {{ detail.draft }}
        </p>
        <div class="mt-3 flex flex-wrap items-center gap-2">
          <button class="rounded-md bg-brand px-3 py-1.5 text-xs font-semibold text-ink-inverse hover:bg-brand-hover" @click="approve">Approve note</button>
      ...
      <div v-else-if="detail?.stage === 'escalated' && !handled" class="border-t border-divider px-5 py-4">
        <div class="text-[11px] font-semibold uppercase tracking-[0.14em] text-danger">{{ detail.channel || 'Personal call' }}</div>
        <p class="mt-2 text-[13px] leading-relaxed text-ink">{{ detail.summary }}</p>
        <button class="mt-3 rounded-md border border-divider px-3 py-1.5 text-xs font-medium text-ink hover:bg-surface-elevated" @click="approve">Mark called</button>
      </div>

      <div v-else-if="handled" class="border-t border-divider px-5 py-4">
        <p class="text-[12px] text-success">Handled. It will come back next week if nothing changes.</p>
      </div>
