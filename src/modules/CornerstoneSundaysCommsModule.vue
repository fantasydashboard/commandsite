...
import { loadCareData, burnoutData, rosterData } from '@/lib/clients/church/careDataLoader'
...
// The SAME roster the hero reads. It used to read the baked focal-point/roster.ts,
// which is generated by hand and was last run on 2026-08-27 against the Aug 30
// plan. So the hero said "Sep 13 is 27 spots short across 7 teams" and the stat
// card directly beneath it said 12 across 6, while the page header claimed
// "Updated 5h ago". Third time a baked file has contradicted the live panel it
// sits next to.
const roster = computed(() => rosterData())
...
const heavyLoadCount = computed(() => heavyLoad(scopedBurnout(burnoutData().people, lens.scope)).length)
    ...
    <template v-if="isFocalPoint">
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div class="card">
          ...
          <div class="mt-1 text-2xl font-bold text-warn tabular-nums">{{ roster.totalShort }}</div>
          <div class="text-[11px] text-ink-disabled mt-0.5">across {{ roster.teamsShort }} teams, as of {{ roster.sundayLabel.replace('Sun ', '') }}</div>
        </div>
        <div class="card">
