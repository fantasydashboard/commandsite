...
async function loadPipeline() {
  ...
  if (err) {
    pipelineError.value = await surfaceFnError(err, 'Pipeline load failed')
    return
  }
...
<template>
  <div class="space-y-6">
      ...
      <section class="card space-y-3">
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <p class="text-xs text-ink-muted">
              Where each UFD trial user is in the lifecycle right now. Column = the last template sent to them. Stalled users (longest in their column) sort first; click any user to drill in.
            </p>
          </div>
          <button
            type="button"
          ...
          <div class="flex gap-3 min-w-max pb-2">
            ...
            <div class="w-[220px] flex-shrink-0 rounded border border-divider bg-surface-elevated/40 p-2 space-y-2">
              ...
              <div class="space-y-1.5 max-h-72 overflow-y-auto">
                ...
                >
                  +{{ pipeline.buckets.not_started.length - 25 }} more
                </div>
                <div
            ...
            >
              ...
              <div class="space-y-1.5 max-h-72 overflow-y-auto">
                ...
                <div
                  ...
                  class="px-1 text-[10px] text-ink-muted"
                >
                  +{{ (pipeline.buckets.by_step_id[step.id]?.length ?? 0) - 25 }} more
                </div>
                <div
            ...
            <div class="w-[220px] flex-shrink-0 rounded border border-success/30 bg-success/5 p-2 space-y-2">
              ...
              <div class="space-y-1.5 max-h-72 overflow-y-auto">
                ...
                <div
                  ...
                  class="px-1 text-[10px] text-ink-muted"
                >
                  +{{ pipeline.buckets.paid.length - 25 }} more
                </div>
                <div
            ...
            <div class="w-[220px] flex-shrink-0 rounded border border-divider bg-surface-elevated/40 p-2 space-y-2">
              ...
              <div class="space-y-1.5 max-h-72 overflow-y-auto">
                ...
                <div
                  ...
                  class="px-1 text-[10px] text-ink-muted"
                >
                  +{{ pipeline.buckets.completed.length - 25 }} more
                </div>
                <div
    ...
    <!-- Pipeline drill-in: shared user detail drawer -->
    <UfdUserDetailDrawer
      :open="pipelineDetailEmail !== null"
      :email="pipelineDetailEmail"
      :client="props.client"
      @close="pipelineDetailEmail = null"
    />
  </div>
</template>
