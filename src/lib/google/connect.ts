...
export async function getGoogleConnection(tenant: string): Promise<GoogleConnectionStatus | null> {
  ...
  return (data as GoogleConnectionStatus | null) ?? null
}
...
export async function disconnectGoogle(tenant: string): Promise<void> {
  const { error } = await sb.from('google_connections').delete().eq('tenant_key', tenant)
