...
const store = reactive({
  ...
  // The Insights page, computed nightly. Six panels there used to come from
  // files I regenerated by hand; the church asked where every number comes from
  // and for half that page the honest answer was "a script, when I remembered".
  // Null until the payload lands, and every consumer falls back to the baked
  // copy so the page renders either way.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
/** Null until the nightly payload lands. Consumers pick their own baked
 *  fallback per panel, because Insights mixes Planning Center data with the
 *  church's weekly sheet and only the former can ever be live. */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const insightsData = (): any | null => store.insights
...
export async function loadCareData(slug: string): Promise<void> {
  ...
  const HEAVY = ['activity', 'duplicates', 'serveCandidates', 'insights']
  ...
  for (const row of rows) {
    ...
    else if (row.module_key === 'insights') store.insights = row.payload
